...
public class SignUpFilter implements Filter {
    ...
    private void doBeforeProcessing(ServletRequest request, ServletResponse response)
        ...
        request.setAttribute("repassword1", repassword);
        if(username.length()<6 || password.length()<6 || repassword.length()<6){
            request.setAttribute("message1", "the length of username or password at least 6 charaters");
            request.getRequestDispatcher("signup.jsp").forward(request, response);
        } 
        
        if(!password.equals(repassword)){
            request.setAttribute("message1", "password and re-password are not conresponding");
            request.getRequestDispatcher("signup.jsp").forward(request, response);
        }
